...
public sealed class VisionSmokeTests(ITestOutputHelper output)
{
    ...
    private const string Lfm25VlModelPath  = @"C:\Users\theo\.lmstudio\models\lmstudio-community\LFM2.5-VL-1.6B-GGUF\LFM2.5-VL-1.6B-Q8_0.gguf";
    private const string Lfm25VlMmprojPath = @"C:\Users\theo\.lmstudio\models\lmstudio-community\LFM2.5-VL-1.6B-GGUF\mmproj-LFM2.5-VL-1.6B-F16.gguf";
    ...
    [Fact]
    [Trait("Category", "LocalModel")]
    public async Task GenerateAsync_WithLfm25VlModel_RecognizesTextInImage()
    {
        Assert.True(File.Exists(Lfm25VlModelPath),  $"Model not found: {Lfm25VlModelPath}");
        Assert.True(File.Exists(Lfm25VlMmprojPath), $"Mmproj not found: {Lfm25VlMmprojPath}");
        Assert.True(File.Exists(SampleImagePath),   $"Sample image not found: {SampleImagePath}");
        ...
        {
            BackendDirectory = backendDir,
            ModelPath        = Lfm25VlModelPath,
            MmprojPath       = Lfm25VlMmprojPath,
            ToolRegistry     = [],
            ContextTokens    = contextTokens,
            Compaction       = new ConversationCompactionOptions(
                MaxInputTokens:        contextTokens,
    ...
    [Fact]
    [Trait("Category", "LocalModel")]
    public async Task GenerateAsync_WithLfm25VlModel_ImageEveryOtherTurn_NeverFullResetOnTextTurn()
    {
        Assert.True(File.Exists(Lfm25VlModelPath),  $"Model not found: {Lfm25VlModelPath}");
        Assert.True(File.Exists(Lfm25VlMmprojPath), $"Mmproj not found: {Lfm25VlMmprojPath}");
        Assert.True(File.Exists(SampleImagePath),   $"Sample image not found: {SampleImagePath}");
